...
export default function ClientDashboardPage() {
  ...
  const { activeServices, completedServices, totalUnpaidBalance, clientMetrics } = data || {};
                ...
                {completedServices?.map((service: any) => (
                  <div key={service.id} className="p-4 flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{new Date(service.endDate).toLocaleDateString()}</p>
                    </div>
